...
def local_background_lab(
    ...
    # 背景通常比胶块亮、色度低。用高 L + 低/中 C 筛掉邻近彩色块污染。
    L = pixels[:, 0]
    C = lab_chroma(pixels)

    L_th = np.percentile(L, 45)
    C_th = np.percentile(C, 70)

    keep = (L >= L_th) & (C <= C_th)
    if keep.sum() < min_pixels:
        # 如果筛太狠，退回到距离中位数最近的一部分
        med = np.median(pixels, axis=0)
        dist = np.linalg.norm(pixels - med[None, :], axis=1)
        idx = np.argsort(dist)[: max(min_pixels, len(pixels) // 2)]
        return np.mean(pixels[idx], axis=0)

    kept = pixels[keep]
    med = np.median(kept, axis=0)
    dist = np.linalg.norm(kept - med[None, :], axis=1)
    idx = np.argsort(dist)[: max(min_pixels, int(len(kept) * 0.75))]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="真实场景 root_poly2 + 局部背景 + ColorChecker residual 校正，不用目标标准值反推补偿。")
    ...
    for item in selected:
        ...
        bg_lab = local_background_lab(root_photo_bgr, roi, all_target_mask=target_mask, margin=args.bg_margin)
