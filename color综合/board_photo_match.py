...
典型运行：
    python board_photo_match.py --photo board.jpg --standard standard_chart.png --library output_128/glue_visual_library/glue_visual_library.csv --mapping output_128/glue_visual_library/visual_mapping_T/visual_mapping_T.json --out board_match_output --top-k 10
...
def make_contact_sheet(
    *,
    ...
    """
    生成 TopK 胶块拼图。
    左侧第一张为 board_visual_lab 色块，后面为推荐胶块 crop。
    """
    n = len(top_results) + 1
    cols = min(5, n)
    ...
    swatch = color_swatch_bgr_from_lab(board_visual_lab, size=(150, 150))
    board_tile[45:195, 35:185] = swatch
    cv2.putText(board_tile, "BOARD visual", (15, 28), font, 0.65, (0, 0, 0), 2)
    cv2.putText(
        board_tile,
        f"L={board_visual_lab[0]:.1f}",
        (15, 220),
        font,
        0.55,
        (0, 0, 0),
    ...
    cv2.putText(
        board_tile,
        f"a={board_visual_lab[1]:.1f} b={board_visual_lab[2]:.1f}",
        (15, 245),
        font,
        0.55,
        (0, 0, 0),
...
def main() -> None:
    ...
    board_visual_bgr = board_visual_rgb[::-1]

    swatch = np.zeros((160, 320, 3), dtype=np.uint8)
    swatch[:, :160] = board_corrected_rgb[::-1].astype(np.uint8)
    swatch[:, 160:] = board_visual_bgr.astype(np.uint8)
    cv2.putText(swatch, "corrected", (18, 35), cv2.FONT_HERSHEY_SIMPLEX, 0.65, (0, 0, 0), 2)
    cv2.putText(swatch, "visual T", (188, 35), cv2.FONT_HERSHEY_SIMPLEX, 0.65, (0, 0, 0), 2)
    imwrite_unicode(out_dir / "04_board_corrected_vs_visual_swatch.png", swatch)
    ...
    print("  色块对比：", out_dir / "04_board_corrected_vs_visual_swatch.png")
