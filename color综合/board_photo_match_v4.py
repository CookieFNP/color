...
def adjust_board_display_lab(
    ...
    参数：
        ...
        chroma_scale:
            a/b 色度倍率。
            灰感主要由色度不足导致，可先试 1.08~1.15。

        chroma_offset:
            固定色度增量。
            当 a/b 本来比较小的时候，单纯乘倍率效果不明显，
            此时可以加 0.5~2.0 的固定色度，沿原 hue 方向增加。
    ...
    for l_off in l_offsets:
        for c_scale in chroma_scales:
            for c_off in chroma_offsets:
                ...
                variants.append(
                    {
                        "name": f"L{l_off:+.1f}_C{c_scale:.2f}_O{c_off:+.1f}",
...
def main() -> None:
    ...
    print("  display adjust: L_offset={}, chroma_scale={}, chroma_offset={}, chroma_max={}".format(
