...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="只用于展示的板材 a/b 色度倍率，不影响 TopK 匹配。发灰时建议 1.08~1.15，默认 1。",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="只用于展示的板材固定色度增量，不影响 TopK 匹配。a/b 很小时单纯倍率不明显，可试 0.5~2.0，默认 0。",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="只用于展示的 a 轴偏移，不影响 TopK。>0 略偏红，<0 略偏绿，默认 0。",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="只用于展示的 b 轴偏移，不影响 TopK。>0 加暖/加黄，<0 偏冷/偏蓝。暖色调不足可试 1~3，默认 0。",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="输出多组板材 display 色块对比图，方便选择更淡/更艳/更亮的展示参数。",
    )

    parser.add_argument(
        "--display-l-offsets",
        default="-0.5,0,0.5",
        help='多版本展示用 L 偏移列表，例如 "-0.5,0,0.5,1.0"。',
    )

    parser.add_argument(
        "--display-chroma-scales",
        default="0.85,0.90,0.95,1.00",
        help='多版本展示用色度倍率列表。想更淡就用小于 1，例如 "0.80,0.85,0.90,0.95,1.00"。',
    )

    parser.add_argument(
        "--display-chroma-offsets",
        default="0",
        help='多版本展示用固定色度增量列表，例如 "-1,0,1"。想更淡可试负值。',
    )

    parser.add_argument(
        "--display-a-offsets",
        default="0,0.4",
        help='多版本展示用 a 轴偏移列表。暖调通常可试 "0,0.3,0.6"。',
    )

    parser.add_argument(
        "--display-b-offsets",
        default="0,1,2,3",
        help='多版本展示用 b 轴偏移列表。暖色调不足时优先试 "0,1,2,3"。',
    )
    ...
    # v6：把框选的 ROI 单独截出来，并输出一张按 display 参数微调后的 ROI 图。
    ...
    result = {
        ...
        "note": "匹配误差 deltaE_match = ΔE(board_visual_lab, glue_visual_display_lab)。display_variants 只用于展示，不参与 TopK 排序。",
    }
