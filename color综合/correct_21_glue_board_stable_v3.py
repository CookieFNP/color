...
def apply_target_residual_to_circles(
    *,
    ...
    dark_ab_scale: float,
    dark_L_offset: float,
    circle_l_offsets: dict[int, float] | None = None,
    circle_a_offsets: dict[int, float] | None = None,
    circle_b_offsets: dict[int, float] | None = None,
    ...
    circle_l_offsets = circle_l_offsets or {}
    circle_a_offsets = circle_a_offsets or {}
    circle_b_offsets = circle_b_offsets or {}
    ...
    for circle_index, circle in enumerate(circles, start=1):
        ...
        manual_L_offset = float(circle_l_offsets.get(circle_index, 0.0))
        manual_a_offset = float(circle_a_offsets.get(circle_index, 0.0))
        manual_b_offset = float(circle_b_offsets.get(circle_index, 0.0))

        # 最后的“按圆编号微调”，不再乘 target_strength。
        # 用于已经接近满意后，只调整某几个胶块。
        residual[0] += manual_L_offset
        residual[1] += manual_a_offset
        residual[2] += manual_b_offset
        ...
        rows.append(
            {
                ...
                "manual_L_offset": float(manual_L_offset),
                "manual_a_offset": float(manual_a_offset),
                "manual_b_offset": float(manual_b_offset),
...
def parse_circle_offsets(text: str) -> dict[int, float]:
    """
    解析按圆编号的手动微调参数。

    格式：
        "5:0.8,6:0.8,8:1.0,9:1.0,18:0.9"
    ...
    for part in parts:
        ...
        if ":" in part:
            ...
            raise ValueError(f"circle offset 格式错误：{part}，应类似 5:0.8")
...
def main() -> None:
    ...
    parser.add_argument(
        "--circle-L-offsets",
        default="",
        help="按圆编号额外调整 L。例：5:0.8,6:0.8,8:1.0。正数=更浅，负数=更深。",
    )
    parser.add_argument(
        "--circle-a-offsets",
        default="",
        help="按圆编号额外调整 a。例：5:-0.2,6:-0.2。正数=更红，负数=更绿。",
    )
    parser.add_argument(
        "--circle-b-offsets",
        default="",
        help="按圆编号额外调整 b。例：5:-0.4,6:-0.4。正数=更黄，负数=黄更淡/偏蓝。",
    )
    ...
    circle_l_offsets = parse_circle_offsets(args.circle_L_offsets)
    circle_a_offsets = parse_circle_offsets(args.circle_a_offsets)
    circle_b_offsets = parse_circle_offsets(args.circle_b_offsets)

    if circle_l_offsets:
        print("按圆编号 L 微调：", circle_l_offsets)
    if circle_a_offsets:
        print("按圆编号 a 微调：", circle_a_offsets)
    if circle_b_offsets:
        print("按圆编号 b 微调：", circle_b_offsets)
    ...
    target_circles_bgr, total_mask, circle_rows = apply_target_residual_to_circles(
        ...
        circle_l_offsets=circle_l_offsets,
        circle_a_offsets=circle_a_offsets,
        circle_b_offsets=circle_b_offsets,
    )
