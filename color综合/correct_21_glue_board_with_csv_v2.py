...
用途：
    ...
    在你已有 target CSV 的情况下做“定点颜色修正”，并且加入
    “浅色保护 + 深色增强”的分段规则，解决：
...
def main() -> None:
    ...
    # 背景
    parser.add_argument("--background-corrected-weight", type=float, default=0.45)
    ...
    parser.add_argument("--light-L-scale", type=float, default=0.60)
    parser.add_argument("--light-ab-scale", type=float, default=0.90)
    parser.add_argument("--light-L-offset", type=float, default=-1.5)

    parser.add_argument("--mid-L-scale", type=float, default=1.00)
    parser.add_argument("--mid-ab-scale", type=float, default=1.00)

    parser.add_argument("--dark-L-scale", type=float, default=1.35)
    parser.add_argument("--dark-ab-scale", type=float, default=1.15)
    ...
    corrected_bgr = apply_color_correction_image(original_bgr, W, model_type=args.model_type, correction_strength=args.correction_strength)
